        ...
        <c:forEach items="${pageInfo.list}" var="user" varStatus="status">
            <tr>
                ...
                <td>
                    ...
                    <a class="btn btn-danger btn-sm" href="${app}/user/delete?uid=${user.uid}"><i class="fa fa-trash"></i> 删除</a>
                </td>
            </tr>
        </c:forEach>
    </tbody>
...
<div class="row">
    <div class="col-12 col-md-8">
        <ul class="pagination">
            <li  class='page-item <c:if test="${pageInfo.isFirstPage}">disabled</c:if>'><a class="page-link" href="${app}/user/list?pageNum=1">首页</a></li>
            <li  class='page-item <c:if test="${not pageInfo.hasPreviousPage}">disabled</c:if>'><a class="page-link" href="${app}/user/list?pageNum=${pageInfo.pageNum-1}">上一页</a></li>
            <c:forEach begin="${pageInfo.navigateFirstPage}" end="${pageInfo.navigateLastPage}" var="i">
                <li  class='page-item <c:if test="${pageInfo.pageNum == i}">active</c:if>'><a class="page-link" href="${app}/user/list?pageNum=${i}">${i}</a></li>
            </c:forEach>
            <li  class='page-item <c:if test="${not pageInfo.hasNextPage}">disabled</c:if>'><a class="page-link" href="${app}/user/list?pageNum=${pageInfo.pageNum+1}">下一页</a></li>
            <li  class='page-item <c:if test="${pageInfo.isLastPage}">disabled</c:if>'><a class="page-link" href="${app}/user/list?pageNum=${pageInfo.pages}">尾页</a></li>
        </ul>
    </div>
